...
import com.nexgrid.adcb.util.Base64Util;
...
import com.nexgrid.apim.module.exception.ApimException;
...
public class ChargeService {
	...
	public void charge(Map<String, Object> paramMap, LogVO logVO) throws Exception {
		...
		// 약관동의가 필요한 경우 ESB/API(MPS 208) 연동
		if("Y".equals(terms_deny_yn)) {
			// ESB/APIM 연동
			// [101049]
			if("esb".equals(Init.readConfig.getUse_esb_apim_mps208())){
	...
	public void doApiMps208(Map<String, Object> paramMap, LogVO logVO) throws Exception {
		...
		data.setEntrId(Base64Util.encodeString(sub_no)); // 가입번호
		...
		try {
			...
			if (response.getHttpStatus().is2xxSuccessful()) { // 일단 요청은 성공
				...
				if (response.getBody().getDsRsltInfo().getRsltCd().equals("SUCCESS")) {
					// 정상적으로 성공시 코드가 SUCCESS 로 넘어옴
					logVO.setApiMps208ResultCode("0000");
				} else if (response.getBody().getDsRsltInfo().getRsltCd().equals("4100")) {
					// SYSTEM ERROR
					throw new CommonException(EnAdcbOmsCode.API_MPS208_4100);
				} else if (response.getBody().getDsRsltInfo().getRsltCd().equals("4004")) {
					// 존재하지 않는 고객
					throw new CommonException(EnAdcbOmsCode.API_MPS208_4004);
				} else if (response.getBody().getDsRsltInfo().getRsltCd().equals("4040")) {
					// 수신 Data 항목오류 (Field 값 validation 오류)
					throw new CommonException(EnAdcbOmsCode.API_MPS208_4040);
				} else if (response.getBody().getDsRsltInfo().getRsltCd().equals("3000")) {
					// Timeout
					throw new CommonException(EnAdcbOmsCode.API_MPS208_TIMEOUT);
				} else {
					// 외 나머지 에러
